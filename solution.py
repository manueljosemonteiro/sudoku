...
def naked_twins(values):
    ...
    naked = [[box1,box2] for box1 in my_twins \
    for box2 in peers[box1] \
    if set(values[box1])==set(values[box2])]
    # Eliminate the naked twins as possibilities for their peers
    # For each pair of naked twins,
    for i in range(len(naked)):
        box1 = naked[i][0] # first digit
        box2 = naked[i][1] # second digit
        # intersect peers
        peers1 = set(peers[box1])
        peers2 = set(peers[box2])
        finded_peers = peers1 & peers2
        # Eliminate the two digits colected from the naked twins in all common peers
        for my_peer in finded_peers:
            if len(values[my_peer])>2:
                for the_val in values[box1]:
                    values = assign_value(values, my_peer, values[my_peer].replace(the_val,'')) #Eliminate values
    return values
